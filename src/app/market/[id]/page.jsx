...
import { checkSufficientBalance, placeBet, handleTransactionCallback } from '@/utils/SolanaWallet.js';
...
export default function MarketPage() {
  ...
  //     if (!data || !nonce) {
  //       return; // Not a callback, skip
  //     }

  //     try {
  //       // Get the signature from the encrypted response
  //       const signature = await handleTransactionCallback(data, nonce);
        
  //       // Get the pending transaction info from localStorage
  //       const pendingAmount = parseFloat(localStorage.getItem('pending_transaction_amount'));
  //       const pendingMarketId = localStorage.getItem('pending_transaction_market_id');
  //       const betType = isPumpActive ? 'PUMP' : 'RUG';

  //       if (!pendingAmount || !pendingMarketId || pendingMarketId !== id) {
  //         throw new Error('Invalid pending transaction data');
  //       }
  ...
  //       const betResponse = await fetch(`/api/betting`, {
  //         method: 'POST',
  //         headers: {
  //           'Content-Type': 'application/json',
  //         },
  ...
  //       // Clean up the URL (remove the callback parameters)
  //       window.history.replaceState({}, '', `/market/${id}`);
  //     } catch (error) {
  ...
  //   handleCallback();
  // }, [searchParams, id, authUser, market, isPumpActive]);
