...
import { Connection, PublicKey, LAMPORTS_PER_SOL, SystemProgram, Transaction, clusterApiUrl } from '@solana/web3.js';
import { logInfo, logError } from '@/utils/logger';
import BN from 'bn.js'; // Add BN import for proper big number handling
...
export async function transferSOL(
  ...
  try {
    ...
    logInfo('Converting amount to lamports', {
      ...
      lamportsCalculated: lamportsAmount.toString(),
      component: 'Solana wallet'
    });
    ...
    if (error.message.includes('User rejected')) {
      ...
      errorMessage = 'Error processing transaction amount. Please try a different amount.';
    }
...
export async function placeBet(
  ...
  try {
    ...
    logInfo('Placing bet with amount', {
      amount: betAmount,
      component: 'Solana wallet'
    });
